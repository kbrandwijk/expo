...
import host.exp.exponent.kernel.KernelProvider.instance
...
class Kernel : KernelInterface() {
  ...
  @Inject
  lateinit var mExponentManifest: ExponentManifest

  @Inject
  lateinit var mExponentSharedPreferences: ExponentSharedPreferences

  @Inject
  lateinit var mExponentNetwork: ExponentNetwork
  ...
  var activityContext: Activity?
    ...
    set(value) {
      if (value != null) {
        mActivityContext = value
      }
    }

  private var mOptimisticActivity: ExperienceActivity? = null

  private var mOptimisticTaskId: Int? = null

  private fun experienceActivityTaskForTaskId(taskId: Int): ExperienceActivityTask? {
    return sManifestUrlToExperienceActivityTask.values.find { it.taskId == taskId }
  }
  ...
  fun startJSKernel(activity: Activity?) {
    ...
    if (!mExponentSharedPreferences.shouldUseInternetKernel()) {
      try {
        ...
        mExponentManifest.kernelManifest
      } catch (e: Throwable) {
    ...
    if (mExponentSharedPreferences.shouldUseInternetKernel() &&
          ...
          Exponent.getInstance().loadJSBundle(
            ...
            object : BundleListener {
              override fun onBundleLoaded(localBundlePath: String) {
                mExponentSharedPreferences.setBoolean(
      ...
      var shouldNotUseKernelCache =
        mExponentSharedPreferences.getBoolean(ExponentSharedPreferences.SHOULD_NOT_USE_KERNEL_CACHE)
      if (!ExpoViewBuildConfig.DEBUG) {
        val oldKernelRevisionId =
          mExponentSharedPreferences.getString(ExponentSharedPreferences.KERNEL_REVISION_ID, "")
  ...
  private fun kernelBundleListener(): BundleListener {
    return object : BundleListener {
      override fun onBundleLoaded(localBundlePath: String) {
        if (!ExpoViewBuildConfig.DEBUG) {
          mExponentSharedPreferences.setString(
        ...
        Exponent.getInstance().runOnUiThread {
          ...
          val builder = ReactInstanceManager.builder()
            ...
            .addPackage(
              ExpoTurboPackage.kernelExpoTurboPackage(
                mExponentManifest.kernelManifest, initialURL
              )
            )
          ...
          if (!KernelConfig.FORCE_NO_KERNEL_DEBUG_MODE && mExponentManifest.kernelManifest.isDevelopmentMode()) {
  ...
  private val kernelDebuggerHost: String
    get() = mExponentManifest.kernelManifest.getDebuggerHost()
  private val kernelMainModuleName: String
    get() = mExponentManifest.kernelManifest.getMainModuleName()
  private val bundleUrl: String?
    get() {
      return try {
        mExponentManifest.kernelManifest.getBundleURL()
  ...
  private val kernelRevisionId: String?
    get() {
      return try {
        mExponentManifest.kernelManifest.getRevisionId()
      } catch (e: JSONException) {
        instance.handleError(e)
        null
      }
  ...
  val isRunning: Boolean
    get() = mIsRunning && !mHasError

  val reactRootView: ReactRootView
    get() {
      val reactRootView: ReactRootView = ReactUnthemedRootView(mContext)
  ...
  private val kernelLaunchOptions: Bundle
    get() {
      ...
      val referrer = mExponentSharedPreferences.getString(ExponentSharedPreferences.REFERRER_KEY)
      if (referrer != null) {
        try {
  ...
  fun addAppLoaderForManifestUrl(manifestUrl: String, appLoader: ExpoUpdatesAppLoader) {
    mManifestUrlToAppLoader[manifestUrl] = appLoader
  }

  override fun getAppLoaderForManifestUrl(manifestUrl: String?): ExpoUpdatesAppLoader? {
    return mManifestUrlToAppLoader[manifestUrl]
  }

  fun getExperienceActivityTask(manifestUrl: String): ExperienceActivityTask {
    var task = sManifestUrlToExperienceActivityTask[manifestUrl]
    if (task != null) {
      return task
    }
    task = ExperienceActivityTask(manifestUrl)
    sManifestUrlToExperienceActivityTask[manifestUrl] = task
    return task
  }
  ...
  fun openHomeActivity() {
    ...
    val intent = Intent(mActivityContext, HomeActivity::class.java)
  ...
  private fun openShellAppActivity(forceCache: Boolean) {
    try {
      ...
      val intent = Intent(mActivityContext, activityClass)
      addIntentDocumentFlags(intent)
      if (forceCache) {
  ...
  fun handleIntent(activity: Activity, intent: Intent) {
      ...
      if (notificationManifestUrl != null) {
        ...
        if (exponentNotification != null) {
          ...
          if (bundle.containsKey(KernelConstants.NOTIFICATION_ACTION_TYPE_KEY)) {
            ...
            val manager = ExponentNotificationManager(mContext)
    ...
    if (uri != null && shouldOpenUrl(uri)) {
      if (Constants.INITIAL_URL == null) {
        ...
        openExperience(ExperienceOptions(intentUri, intentUri, null))
        return
      } else {
        ...
        openExperience(ExperienceOptions(Constants.INITIAL_URL, intentUri, null))
        return
      }
    }
    openDefaultUrl()
  ...
  private fun openManifestUrl(
    ...
    if (options == null) {
      ...
      mManifestUrlToOptions[manifestUrl] = options
    }
      ...
      ExpoUpdatesAppLoader(
        ...
        object : AppLoaderCallback {
          ...
          override fun emitEvent(params: JSONObject) {
            val task = sManifestUrlToExperienceActivityTask[manifestUrl]
            if (task != null) {
  ...
  private fun openOptimisticExperienceActivity(manifestUrl: String?) {
    try {
      val intent = Intent(mActivityContext, ExperienceActivity::class.java).apply {
  ...
  override fun reloadVisibleExperience(manifestUrl: String?, forceCache: Boolean): Boolean {
    ...
    for (experienceActivityTask: ExperienceActivityTask in sManifestUrlToExperienceActivityTask.values) {
  ...
  companion object {
    ...
    private lateinit var sInstance: Kernel

    // Activities/Tasks
    private val sManifestUrlToExperienceActivityTask = mutableMapOf<String, ExperienceActivityTask>()
    private val mManifestUrlToOptions = mutableMapOf<String?, ExperienceOptions>()
    private val mManifestUrlToAppLoader = mutableMapOf<String?, ExpoUpdatesAppLoader>()
    ...
    fun getManifestUrlForActivityId(activityId: Int): String? {
      return sManifestUrlToExperienceActivityTask.values.find { it.activityId == activityId }?.manifestUrl
    }
    ...
    fun getBundleUrlForActivityId(
      ...
      return sManifestUrlToExperienceActivityTask.values.find { it.activityId == activityId }?.bundleUrl
    }
    ...
    fun getBundleUrlForActivityId(
      ...
      return sManifestUrlToExperienceActivityTask.values.find { it.activityId == activityId }?.let { task ->
